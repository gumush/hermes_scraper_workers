...
class ImageHandler:
    ...
    def download_place_photos(self, place_id: str,
                              photos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Download owner-uploaded place photos to
        {image_dir}/places/{place_id}/owner/. Mutates and returns `photos`,
        adding a `local_path` key (relative to image_dir) per downloaded item.

        Uses the same hardened session as review images (Referer, UA,
        forwarded browser cookies, retries) and the same host allowlist.
        """
        if not photos:
            return photos

        safe_place = _sanitize_filename(place_id.replace(":", "_"))
        target_dir = self.image_dir / "places" / safe_place / "owner"
        target_dir.mkdir(parents=True, exist_ok=True)

        def _download_one(photo: Dict[str, Any]) -> None:
            url = photo.get("url_large") or photo.get("url") or ""
            if not url or not _is_allowed_image_host(url):
                return
            # stable filename from the CDN token tail
            token = url.split("/")[-1].split("=")[0]
            filename = f"{photo.get('index', 0):03}_{_sanitize_filename(token[-24:])}.jpg"
            filepath = target_dir / filename
            rel_path = str(filepath.relative_to(self.image_dir))
            if filepath.exists():
                photo["local_path"] = rel_path
                return
            try:
                response = self._session.get(url, stream=True, timeout=15)
                if response.status_code == 403:
                    self._forbidden_count += 1
                    return
                response.raise_for_status()
                ...
                log.debug(f"Error downloading place photo {url}: {e}")

        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            list(executor.map(_download_one, photos))

        done = sum(1 for p in photos if p.get("local_path"))
        log.info(f"Downloaded {done}/{len(photos)} owner photos to {target_dir}")
        return photos
