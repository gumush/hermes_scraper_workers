...
DEFAULT_CONFIG = {
    ...
    "businesses": [],
    "headless": True,
    "language": "en",    # Google Maps UI language (hl param + Chrome locale)
    "region": "tr",      # gl param: country bias, so a VM abroad still sees
                         # the same results a local request would
    "sort_by": "relevance",
    "scrape_mode": "update",
    "stop_on_match": False,
    ...
    "scrape_place_details": True,   # Extract business info (hours, price, about)
    "place_details_json_path": "place_details.json",
    "scrape_place_photos": True,    # Collect "By owner" photo URLs from the gallery
    "place_photos_limit": 60,       # Max owner photos to collect per place
    "download_place_photos": True,  # Download owner photos to {image_dir}/places/
    "use_mongodb": True,
    "mongodb": {
