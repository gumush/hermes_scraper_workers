...
from workers.engine.database_backend import SQLiteBackend
from workers.engine.place_id import canonicalize_url
...
-- Place details (business info beyond reviews: hours, price, about, contact)
...
class ReviewDB:
    ...
    def upsert_place_details(self, place_id: str, details: Dict[str, Any]) -> None:
        """Store or replace the place-details document for a place."""
        now = _now_utc()
        self.backend.execute(
            "INSERT INTO place_details (place_id, details, updated_at) "
            "VALUES (?, ?, ?) "
            "ON CONFLICT(place_id) DO UPDATE SET details = excluded.details, "
            "updated_at = excluded.updated_at",
            (place_id, json.dumps(details, ensure_ascii=False), now)
        )
        # Reflect authoritative header values onto the places row so
        # db-stats / API listings show real name and total review count.
        name = details.get("name")
        review_count = details.get("review_count")
        if name:
            self.backend.execute(
                "UPDATE places SET place_name = ? WHERE place_id = ?",
                (name, place_id)
            )
    ...
    def get_place_details(self, place_id: str) -> Optional[Dict[str, Any]]:
        """Get the stored place-details document (checks aliases too)."""
        place = self.get_place(place_id)
        canonical_id = place["place_id"] if place else place_id
        row = self.backend.fetchone(
            "SELECT details, updated_at FROM place_details WHERE place_id = ?",
            (canonical_id,)
        )
        if not row:
            return None
        try:
            ...
            return None
        details["_updated_at"] = row["updated_at"]
        return details

    def list_place_details(self) -> Dict[str, Dict[str, Any]]:
        """All place-details documents keyed by place_id."""
        rows = self.backend.fetchall("SELECT place_id, details FROM place_details")
        out: Dict[str, Dict[str, Any]] = {}
        for r in rows:
            try:
                out[r["place_id"]] = json.loads(r["details"])
            except (ValueError, TypeError):
                continue
        return out
    ...
    def _build_owner_dict(review: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if owner_text:
            from workers.engine.utils import detect_lang
...
_MIGRATIONS: Dict[int, List[str]] = {
    ...
    # v3: place_details table (business info beyond reviews). Additive only.
    3: [
        """CREATE TABLE IF NOT EXISTS place_details (
    place_id       TEXT PRIMARY KEY,
    details        TEXT NOT NULL,
