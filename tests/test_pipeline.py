...
from workers.engine.pipeline import (
    CleanupTask,
    CustomParamsTask,
    DateTask,
    ImageTask,
...
class TestImageTask:
    ...
    @patch("workers.engine.pipeline.ImageHandler")
...
class TestS3Task:
    ...
    @patch("workers.engine.pipeline.S3Handler")
...
class TestMongoDBTask:
    ...
    @patch("workers.engine.pipeline.MongoDBStorage" if False else "workers.engine.data_storage.MongoDBStorage")
    def test_run_calls_write_reviews(self, mock_storage_cls):
        ...
        with patch("workers.engine.data_storage.MongoDBStorage", mock_storage_cls):
            task.run(reviews, "p1")
